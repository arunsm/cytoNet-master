...
function betw = node_betweenness_faster(G, adj)
...
for i=1:n
    ...
    [dist, P, ~] = graphshortestpath(G, i, 'Directed' , 'false');
    for j=1:n
      
        if dist(j)<=1; continue; end   % either i=j or i,j are 1 edge apart
        
        if ~isfinite(dist(j)); continue; end; % for infinite path lengths
...
betw=betw/nchoosek(n-1,2);   % further normalize by the number of all node pairs excluding the vertex itself